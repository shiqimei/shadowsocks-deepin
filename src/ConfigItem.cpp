...
ConfigItem::ConfigItem(QJsonObject o)
{
    ...
    textPadding = 5;
    config=o;
    texts<<getName()<<getServer()<<getStatus()<<getLatency()<<getLocalPort()<<getTermUsage()<<getTotalUsage()<<getResetDate()<<getLastUsed();
//    qDebug()<<texts;
}
...
void ConfigItem::drawCell(bool isSelect, QRect rect, QPainter *painter,QString text)
{
    ...
    painter->drawText(QRect(rect.x(), rect.y(), rect.width() - textPadding, rect.height()), Qt::AlignRight | Qt::AlignVCenter, text);
}
